...
import { useCreateParty } from "@/lib/hooks";
import { ArrowLeft, Save, Loader2 } from "lucide-react";
import Link from "next/link";

export default function NewPartyPage() {
  const router = useRouter();
  const { role, agent } = useAuthStore();
  const createParty = useCreateParty();

  const [formData, setFormData] = useState({
    account_name: "",
    access_code: "PTY-" + Math.random().toString(36).substring(2, 8).toUpperCase(),
    address: "",
    city: "",
    pin_code: "",
    state: "",
  ...
  if (!role) {
    return (
      <div className="flex flex-col items-center justify-center h-full p-8 text-center bg-background">
        ...
        <p className="text-sm text-text-muted mb-6">Please log in to add a party.</p>
        <Link
          href="/login"
  ...
  return (
    <div className="p-4 lg:p-8 max-w-2xl mx-auto min-h-screen">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-2xl font-bold text-text-primary">Add New Party</h1>
          <p className="text-sm text-text-muted mt-1">Register a new customer or delivery location</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-surface border border-border rounded-xl p-5 md:p-6 shadow-sm space-y-5">
        ...
        <div className="pt-4 border-t border-border flex justify-end gap-3">
          ...
          <button
            ...
            disabled={createParty.isPending || !formData.account_name.trim()}
          >
